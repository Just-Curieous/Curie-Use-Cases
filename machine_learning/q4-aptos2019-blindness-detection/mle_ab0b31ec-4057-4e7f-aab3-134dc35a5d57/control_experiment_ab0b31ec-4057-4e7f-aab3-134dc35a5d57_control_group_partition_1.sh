...
# Simplified Control script for diabetic retinopathy detection experiment
...
export VENV_PATH="/workspace/mle_ab0b31ec-4057-4e7f-aab3-134dc35a5d57/venv"
micromamba activate $VENV_PATH/

# Define paths
WORKSPACE_DIR="/workspace/mle_ab0b31ec-4057-4e7f-aab3-134dc35a5d57"
...
LOGS_FILE="${WORKSPACE_DIR}/results_ab0b31ec-4057-4e7f-aab3-134dc35a5d57_control_group_partition_1.txt"

# Create output directory
mkdir -p $OUTPUT_DIR

# Create a simplified training script for the experiment
cat > ${WORKSPACE_DIR}/simplified_train.py << 'EOF'
import os
import argparse
import pandas as pd
import numpy as np
...
# Start experiment
{
    echo "===== Starting Simplified Diabetic Retinopathy Detection Experiment ====="
    echo "Experiment ID: ab0b31ec-4057-4e7f-aab3-134dc35a5d57"
    ...
    $VENV_PATH/bin/python -c "import torch; print(f'CUDA available: {torch.cuda.is_available()}'); print(f'CUDA device count: {torch.cuda.device_count()}' if torch.cuda.is_available() else 'No CUDA devices'); print(f'CUDA device name: {torch.cuda.get_device_name(0)}' if torch.cuda.is_available() else 'No CUDA devices')"
    echo ""
    ...
    echo "Train images directory: $TRAIN_IMG_DIR"
    echo "Test images directory: $TEST_IMG_DIR"
    echo "Train images count: $(ls $TRAIN_IMG_DIR | wc -l)"
    echo "Test images count: $(ls $TEST_IMG_DIR | wc -l)"
    echo ""
...
"
    ...
    # Train model with simplified script (2 epochs)
    echo "===== Training Model (Simplified, 2 epochs) ====="
    $VENV_PATH/bin/python ${WORKSPACE_DIR}/simplified_train.py \
        --train_csv $TRAIN_CSV \
        --test_csv $TEST_CSV \
    ...
    echo "Training: 2 epochs only"
    echo "Preprocessing: CLAHE enhancement"
    echo "Augmentation: Minimal (horizontal/vertical flips, small rotations)"
    echo "Class balancing: Weighted loss function"
    echo "DataLoader: num_workers=0 (fixed to prevent bus error)"
    echo ""
    
    echo "Results:"
    if [ -f "$OUTPUT_DIR/validation_results_metrics.csv" ]; then
        echo "Validation metrics:"
        cat $OUTPUT_DIR/validation_results_metrics.csv
    else
        echo "Validation metrics file not found."
    fi
    echo ""
    ...
    echo "Submission file: $OUTPUT_DIR/submission.csv"
    echo "Model saved after each epoch and best model saved to: $OUTPUT_DIR/best_model.pt"

} 2>&1 | tee $LOGS_FILE

echo "Experiment completed. Logs saved to $LOGS_FILE"