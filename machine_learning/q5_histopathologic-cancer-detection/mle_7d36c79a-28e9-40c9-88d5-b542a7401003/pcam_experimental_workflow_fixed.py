#!/usr/bin/env python
# PatchCamelyon (PCam) Cancer Detection Experimental Workflow
# Comparing 5 different model architectures on the PatchCamelyon dataset
...
# Simple CNN model for when pretrained models are not available
    ...
    try:
        if config_name == "ResNet50":
            ...
            try:
                ...
                # Fallback to a simple CNN if EfficientNet is not available
                log(f"EfficientNet not available in this PyTorch version. Using SimpleCNN instead.")
                model = SimpleCNN()
        elif config_name == "SEResNeXt50":
            try:
                # Try to use torchvision's ResNeXt
                ...
                # Fallback to a simple CNN if ResNeXt is not available
                log(f"SEResNeXt not available in this PyTorch version. Using SimpleCNN instead.")
                model = SimpleCNN()
        ...
        log(f"Error creating model {config_name}: {str(e)}")
        log(f"Falling back to SimpleCNN")
        model = SimpleCNN()
...
def create_optimizer_scheduler(model, config_name, epochs):
    if config_name in ["ResNet50", "DenseNet121", "EfficientNetB0", "SEResNeXt50"]:
        ...
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.001, epochs=epochs, steps_per_epoch=1)
    else:
...
def run_experiment(config_name, batch_size, epochs, use_mixup=False):
    ...
    try:
        ...
        log(f"Error in experiment {config_name}: {str(e)}")
        import traceback
        ...
        return {
            ...
            "Inference Time per Sample (ms)": 0.0,
            "Model Size (MB)": 0.0,
            "Error": str(e)
        }
...
def main():
    ...
    for config in configurations:
        try:
            ...
            log(f"Failed to run experiment for {config['name']}: {str(e)}")
            all_results.append({
                "Model": config["name"],
                "AUC-ROC": 0.0,
                "Training Time (s)": 0.0,
